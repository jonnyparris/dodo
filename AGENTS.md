...
- `typecheck` — runs `tsc --noEmit` against the workspace inside the CodingAgent DO. No subprocess, no native binaries: bundles `typescript` + every `lib.*.d.ts` into the Worker. Honours user `tsconfig.json` if present. Refuses oversized projects (> 50 .ts/.tsx files or > 5 MB) with a structured `skipped` payload so the model gets actionable feedback instead of a DO crash.

The lib map (`src/typecheck-libs.generated.ts`) is regenerated by `scripts/generate-typecheck-libs.mjs` as part of `npm run build` and **checked in** (3 MB but only churns on a TypeScript version bump, and CI needs it for `tsc --noEmit` to resolve the dynamic import). Validate the typecheck path manually with `npm run test:typecheck-smoke` — a six-case Node script that bypasses vitest-pool-workers' 1 MB WebSocket cap, which can't ship the 3 MB lib map across its result channel.
