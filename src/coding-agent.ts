...
const SYSTEM_PROMPT = [
  ...
  "git_branch, git_checkout, git_diff, git_log, git_verify_remote_branch, pr_create.",
  ...
  "- **Open the PR before you reply.** When the user asks for a PR: push the branch with `git_push_checked`, then call `pr_create` to open a draft PR/MR and quote the URL it returns. `pr_create` works for GitHub and GitLab, auto-detects the provider, and auto-fills the title and body from your latest commit. Don't make the user ask \"where's the PR?\" — that's a workflow failure. If `pr_create` fails (e.g. missing token), fall back to constructing `https://github.com/<owner>/<repo>/compare/<base>...<branch>?expand=1` and tell the user what's missing.",
