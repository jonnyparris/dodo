  ...
  override getSystemPrompt(): string {
    ...
    if (browserEnabled) {
      prompt += [
        ...
        "1. Get available targets: `cdp.send(\"Target.getTargets\")`",
        "2. Attach to a page target: `cdp.attachToTarget(targetId)` → returns a `sessionId`",
        "3. Enable domains: `cdp.send(\"Page.enable\", {}, { sessionId })`",
        "4. Run commands: `cdp.send(\"Page.navigate\", { url }, { sessionId })`",
        "5. All target-scoped commands need `{ sessionId }` in options",
        "",
        "**Common tasks:**",
