...
export function createShellTool(workspace: Workspace) {
  return tool({
    description: [
      ...
      "",
      "Each call boots a fresh sandbox with `/workspace` mounted read+write (file changes flush back to the Workspace on close). Use this for file-shaped work that's awkward in `codemode`:",
      "",
      ...
      "What's available:",
      "",
      "- `sh` (busybox's `hush`), all the standard coreutils applets: `cat`, `ls`, `cp`, `mv`, `rm`, `mkdir`, `find`, `grep`, `sed`, `awk`, `head`, `tail`, `wc`, `sort`, `uniq`, `tr`, `cut`, `xargs`, `tar`, `gzip`. Run `busybox --list` from the shell to see the full set.",
      "- `/workspace` — your session workspace, read+write. **Use absolute paths starting with `/workspace`** — the default working directory is `/workspace` but explicit paths are clearer.",
      ...
      "",
      "Output is capped at 32 KB combined across all commands. stdout and stderr currently merge into `stdout` (a future iteration will split them). If your output is truncated, narrow the query — `grep -m 20`, `head`, `find … -maxdepth 2`, etc.",
      "",
      "Typical use:",
      "",
      "```",
