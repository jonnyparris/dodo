...
// --- Config ---
async function loadConfig(){const cfg=await apiSafe("/api/config");if(!cfg)return;$("cfg-model").value=cfg.model;$("cfg-gateway").value=cfg.activeGateway;const gwUrl=cfg.activeGateway==="opencode"?cfg.opencodeBaseURL:cfg.aiGatewayBaseURL;$("cfg-gateway-url").value=gwUrl||"";$("cfg-git-author-name").value=cfg.gitAuthorName||"";$("cfg-git-author-email").value=cfg.gitAuthorEmail||"";const prefixField=$("cfg-system-prompt-prefix");if(prefixField)prefixField.value=cfg.systemPromptPrefix||"";const prefixSummary=cfg.systemPromptPrefix?`<div class="kv"><span>Prompt prefix</span><code>${esc(cfg.systemPromptPrefix.length+" chars")}</code></div>`:"";$("config-display").innerHTML=`<div class="kv"><span>Model</span><code>${esc(cfg.model)}</code></div><div class="kv"><span>Gateway</span><code>${esc(cfg.activeGateway)}</code></div><div class="kv"><span>Git Author</span><code>${esc(cfg.gitAuthorName)} &lt;${esc(cfg.gitAuthorEmail)}&gt;</code></div>${prefixSummary}`}
async function saveConfig(){const gw=$("cfg-gateway").value;const body={model:$("cfg-model").value,activeGateway:gw,gitAuthorName:$("cfg-git-author-name").value.trim(),gitAuthorEmail:$("cfg-git-author-email").value.trim()};const url=$("cfg-gateway-url").value.trim();if(url){if(gw==="opencode")body.opencodeBaseURL=url;else body.aiGatewayBaseURL=url}const prefixField=$("cfg-system-prompt-prefix");if(prefixField){const prefix=prefixField.value;if(prefix.length>4000){toast(`Prompt prefix too long (${prefix.length}/4000 chars)`,'error');return}body.systemPromptPrefix=prefix}const r=await jsonSafe("/api/config",body,"PUT");if(r)toast('Config saved','success');else toast('Failed to save config','error');await loadConfig()}
