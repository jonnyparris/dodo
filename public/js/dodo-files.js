...
// --- Config ---
async function loadConfig(){const cfg=await apiSafe("/api/config");if(!cfg)return;$("cfg-model").value=cfg.model;$("cfg-gateway").value=cfg.activeGateway;const gwUrl=cfg.activeGateway==="opencode"?cfg.opencodeBaseURL:cfg.aiGatewayBaseURL;$("cfg-gateway-url").value=gwUrl||"";$("cfg-git-author-name").value=cfg.gitAuthorName||"";$("cfg-git-author-email").value=cfg.gitAuthorEmail||"";const imgField=$("cfg-image-model");if(imgField)imgField.value=cfg.replicateImageModel||"";const prefixField=$("cfg-system-prompt-prefix");if(prefixField)prefixField.value=cfg.systemPromptPrefix||"";const prefixSummary=cfg.systemPromptPrefix?`<div class="kv"><span>Prompt prefix</span><code>${esc(cfg.systemPromptPrefix.length+" chars")}</code></div>`:"";const imgSummary=cfg.replicateImageModel?`<div class="kv"><span>Image model</span><code>${esc(cfg.replicateImageModel)}</code></div>`:"";$("config-display").innerHTML=`<div class="kv"><span>Model</span><code>${esc(cfg.model)}</code></div><div class="kv"><span>Gateway</span><code>${esc(cfg.activeGateway)}</code></div>${imgSummary}<div class="kv"><span>Git Author</span><code>${esc(cfg.gitAuthorName)} &lt;${esc(cfg.gitAuthorEmail)}&gt;</code></div>${prefixSummary}`}
