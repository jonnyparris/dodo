...
// --- Config ---
async function loadConfig(){const cfg=await apiSafe("/api/config");if(!cfg)return;$("cfg-model").value=cfg.model;$("cfg-gateway").value=cfg.activeGateway;const gwUrl=cfg.activeGateway==="opencode"?cfg.opencodeBaseURL:cfg.aiGatewayBaseURL;$("cfg-gateway-url").value=gwUrl||"";$("cfg-git-author-name").value=cfg.gitAuthorName||"";$("cfg-git-author-email").value=cfg.gitAuthorEmail||"";const imgField=$("cfg-image-model");if(imgField){imgField.value=cfg.replicateImageModel||"";if(typeof loadReplicateModels==='function')loadReplicateModels()}const prefixField=$("cfg-system-prompt-prefix");if(prefixField)prefixField.value=cfg.systemPromptPrefix||"";const prefixSummary=cfg.systemPromptPrefix?`<div class="kv"><span>Prompt prefix</span><code>${esc(cfg.systemPromptPrefix.length+" chars")}</code></div>`:"";const imgSummary=cfg.replicateImageModel?`<div class="kv"><span>Image model</span><code>${esc(cfg.replicateImageModel)}</code></div>`:"";$("config-display").innerHTML=`<div class="kv"><span>Model</span><code>${esc(cfg.model)}</code></div><div class="kv"><span>Gateway</span><code>${esc(cfg.activeGateway)}</code></div>${imgSummary}<div class="kv"><span>Git Author</span><code>${esc(cfg.gitAuthorName)} &lt;${esc(cfg.gitAuthorEmail)}&gt;</code></div>${prefixSummary}`}
async function saveConfig(){const gw=$("cfg-gateway").value;const body={model:$("cfg-model").value,activeGateway:gw,gitAuthorName:$("cfg-git-author-name").value.trim(),gitAuthorEmail:$("cfg-git-author-email").value.trim()};const url=$("cfg-gateway-url").value.trim();if(url){if(gw==="opencode")body.opencodeBaseURL=url;else body.aiGatewayBaseURL=url}const imgField=$("cfg-image-model");if(imgField)body.replicateImageModel=imgField.value.trim();const prefixField=$("cfg-system-prompt-prefix");if(prefixField){const prefix=prefixField.value;if(prefix.length>4000){toast(`Prompt prefix too long (${prefix.length}/4000 chars)`,'error');return}body.systemPromptPrefix=prefix}const r=await jsonSafe("/api/config",body,"PUT");if(r)toast('Config saved','success');else toast('Failed to save config','error');await loadConfig()}
...
// --- Watchdog ---
// Tiny relative-time formatter for "last fired Xm ago" style status lines.
function _watchdogAgo(epoch){if(!epoch)return"";const s=Math.max(0,Math.floor(Date.now()/1000)-Number(epoch));if(s<60)return`${s}s ago`;if(s<3600)return`${Math.floor(s/60)}m ago`;return`${Math.floor(s/3600)}h ago`}
async function loadWatchdog(){if(!currentSession){$("watchdog-status").innerHTML='<div class="empty">No session</div>';return}try{const s=await api(`/session/${currentSession}/watchdog`);if(!s.armed){$("watchdog-status").innerHTML='<div class="empty">Not armed</div>';return}const cfg=s.config||{};const lastFired=s.lastFiredAt?` · fired ${_watchdogAgo(s.lastFiredAt)}`:"";const lastChecked=s.lastCheckedAt?` · checked ${_watchdogAgo(s.lastCheckedAt)}`:"";$("watchdog-status").innerHTML=`<div class="kv"><span title="${esc(cfg.checkCron||"")}">🛡 ${esc(cfg.action||"notify")} after ${cfg.stallSeconds||"?"}s${lastChecked}${lastFired}</span><button onclick="disarmWatchdog()" class="sm">x</button></div>`}catch{$("watchdog-status").innerHTML='<div class="empty">Not armed</div>'}}
async function armWatchdog(action){if(!currentSession)return;const stall=parseInt($("watchdog-stall").value,10)||600;const r=await jsonSafe(`/session/${currentSession}/watchdog`,{stallSeconds:stall,action},"PUT");if(r)toast(`Watchdog armed (${action})`,'success');else toast('Failed to arm watchdog','error');await loadWatchdog()}
async function disarmWatchdog(){if(!currentSession)return;await apiSafe(`/session/${currentSession}/watchdog`,{method:"DELETE"});toast('Watchdog disarmed','success');await loadWatchdog()}
